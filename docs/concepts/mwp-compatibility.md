...
This kit adapts those ideas into a tool-agnostic folder workspace and supports two operating modes.

## 1. General folder workflows

Used by the `minimal` template and any workspace extended from it.
...
_workspace/runs/archive/<run-slug>/   Explicitly archived finished run
```
...
| `output/` | standalone-task/workflow drafts under run `output/`; pipeline stage artifacts under run `stages/`; finished durable artifacts under workspace `_workspace/outputs/` |
