...
PRIMER_PATH = KIT_ROOT / "docs" / "folder-workflow-system-primer.md"
REGISTRY_PATH = KIT_ROOT / "state" / "workspaces.json"
AVAILABLE_TEMPLATES = ["minimal", "development", "devops"]
...
def main() -> int:
    ...
    if template == "development":
        print(f"  5. Browse _workspace/workflows/ — pre-built dev workflows are ready to use.")
        print(f"  6. Create a pipeline: copy _workspace/pipelines/pipeline-template/ into")
        ...
        print(f"     _workspace/pipelines/<your-pipeline-name>/ and customize stage contracts.")
    print(f"\n  Verify: python3 {KIT_ROOT / 'scripts' / 'verify-workspace.py'} {target_dir}")
    print(f"  Registry: python3 {KIT_ROOT / 'scripts' / 'workspaces.py'} list")
